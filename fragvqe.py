...
def apply_single_qubit_gate(state, qubit, n_qubits, gate):
    ...
    n = len(state)
    state_reshaped = state.reshape(-1, 2, 2**qubit)
    new_state = np.empty_like(state_reshaped)
    new_state[:, 0, :] = gate[0, 0] * state_reshaped[:, 0, :] + gate[0, 1] * state_reshaped[:, 1, :]
    new_state[:, 1, :] = gate[1, 0] * state_reshaped[:, 0, :] + gate[1, 1] * state_reshaped[:, 1, :]
...
def apply_cnot(state, control, target, n_qubits):
    ...
    # Vectorized CNOT: swap amplitudes where control bit is 1
    idx = np.arange(n)
    mask_control_1 = (idx >> control) & 1 == 1
    idx_target_0 = idx[mask_control_1 & ((idx >> target) & 1 == 0)]
    idx_target_1 = idx_target_0 ^ (1 << target)
    new_state[idx_target_0] = state[idx_target_1]
    new_state[idx_target_1] = state[idx_target_0]
    return new_state
...
def fci_exact_energy(qubit_hamiltonian, n_qubits):
    ...
    """
    sparse_H = get_sparse_operator(qubit_hamiltonian, n_qubits=n_qubits)
    # For small systems (N <= 16, i.e., 4 qubits), dense diagonalization is safer and faster.
    if sparse_H.shape[0] <= 16:
        return float(np.min(np.linalg.eigvalsh(sparse_H.toarray())))
    
    # For larger systems, use the sparse solver
    eigenvalues, _ = eigsh(sparse_H, k=1, which='SA', maxiter=10000)
